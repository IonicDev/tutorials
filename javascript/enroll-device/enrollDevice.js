...
const main = async () => {
  ...
  if (keyspace === '') {
    console.log('Didn\'t set keyspace in jssdkConfig.js');
    return;
  }

  if (tenantId === '') {
    // add code to get tenant ID via keyspace.
    console.log('Didn\'t set tenantId in jssdkConfig.js');
    return;
  }

  // Get the appData and add enrollment URL.
  let appData = getAgentConfig('Javascript Enroll Device');
  appData = Object.assign(appData, {enrollmentUrl: enrollmentUrl});
